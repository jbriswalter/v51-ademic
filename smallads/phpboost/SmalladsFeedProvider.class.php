...
class SmalladsFeedProvider implements FeedProvider
{
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		if (SmalladsService::get_categories_manager()->get_categories_cache()->category_exists($idcat))
		{
			...
			$feed_module_name = LangLoader::get_message('feed.name', 'common', 'smallads');
			$data = new FeedData();
			...
			$now = new Date();
			$results = $querier->select('SELECT smallads.id, smallads.id_category, smallads.name, smallads.rewrited_name, smallads.contents, smallads.short_contents, smallads.creation_date, smallads.picture_url, cat.rewrited_name AS rewrited_name_cat
				...
				ORDER BY smallads.creation_date DESC', array(
			...
			foreach ($results as $row)
			{
				...
				$link = SmalladsUrlBuilder::display_smallad($row['id_category'], $row['rewrited_name_cat'], $row['id'], $row['rewrited_name']);
