...
class SmalladsSearchable extends AbstractSearchableExtensionPoint
{
	public function get_search_request($args)
	{
		...
		return "SELECT " . $args['id_search'] . " AS id_search,
			...
			CONCAT('" . PATH_TO_ROOT . "/smallads/" . (!ServerEnvironmentConfig::load()->is_url_rewriting_enabled() ? "index.php?url=/" : "") . "', id_category, '-', IF(id_category != 0, cat.rewrited_name, 'root'), '/', n.id, '-', n.rewrited_name) AS link
			FROM " . SmalladsSetup::$smallads_table . " n
			LEFT JOIN ". SmalladsSetup::$smallads_cats_table ." cat ON n.id_category = cat.id
